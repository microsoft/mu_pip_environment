...
def AddEntriesFromConfig(config, input_config_file):
    ...
    if "remotes" in content:
        for remote in content["remotes"]:
            currentRemoteName = config.NameForUrl(remote["url"])
            if (currentRemoteName != None):
                if (remote["name"] != currentRemoteName):
                    logging.debug("remote with name: {0} already in cache, renaming to {1}".format(currentRemoteName, remote["name"]))
